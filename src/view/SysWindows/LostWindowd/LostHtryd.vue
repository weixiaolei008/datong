...
<script>
  ...
  import {lostPlandHistoryApi, lostHistoryExportApi} from "@/common/config/api/SysWindows"
  ...
  export default {
    ...
    methods: {
      ...
      //列表
      hydPlanList(){
        lostPlandHistoryApi(this.fromVal(1)).then(res => {
          if(res.success) {
      ...
      exportBtn() {
        lostHistoryExportApi(this.fromVal(2)).then(res => {
          if(res.repData){
