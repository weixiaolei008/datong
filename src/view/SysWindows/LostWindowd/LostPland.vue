...
<script>
  ...
  import {lostPlandListApi, lostPlandCreatApi, lostPlandReceivecarApi, lostPlandDeleteApi, lostPlandExportApi, lostPlandImportApi} from "@/common/config/api/SysWindows"
    ...
    methods: {
      ...
      receiveCarBtn() {
        if(this.checkedChange('3')){
          confirmPop('确定接收车辆', () => {
            lostPlandReceivecarApi({listIds: this.checkedChange('3')}).then(res => {
              this.hydPlanList();
              this.feedMessage('warning', res.repData)
            })
          })
      ...
      deltFactory() {
        if(this.checkedChange()){
          confirmPop('确认删除', () => {
            lostPlandDeleteApi({listIds: this.checkedChange()}).then(res => {
              this.feedMessage('warning',res.repData);
              this.hydPlanList()
            })
          })
        ...
        if(num == 2) {
          ...
          if(this.tableSelectId.length > 0){
            this.tableSelectId.forEach((item) => {
              arr.push(item.ttWindowLackId);
            })
          }
          p.ttWindowLackIds = arr.length > 0 ? arr : null;
        }
        return p;
      },
      //列表
      hydPlanList(){
       lostPlandListApi(this.fromVal(1)).then(res => {
        if(res.repData) {
      ...
      exportBtn() {
        lostPlandExportApi(this.fromVal(2)).then(res => {
          if(res.repData){
      ...
      importExcal(res) {
        ...
        lostPlandImportApi(p).then(res => {
          res && this.feedMessage('warning',res.repData);
          this.hydPlanList();
        })
      }
    },
