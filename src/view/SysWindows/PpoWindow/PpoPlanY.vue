...
<script>
  ...
  import {ppoPlanListApi, ppoPlanExportApi, ppoPlanDeliveryCarApi} from "@/common/config/api/SysWindows"
    ...
    methods: {
      ...
      liftCarBtn() {
        if(this.checkedChange()){
          confirmPop('确定通知提车', () => {
            ppoPlanDeliveryCarApi(this.checkedChange()).then(res => {
              this.ppoPlanList();
