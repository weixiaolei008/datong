...
<script>
  ...
  import { getSplitString, exportFile, timestampToTime, confirmPop }from "@/common/config/utils"
  import {svoPlanListApi, svoPlanCreateApi, svoPlanReceivecarApi, svoPlanDeleteApi, svoPlanExportApi, svoPlanImportApi} from "@/common/config/api/SysWindows"
  ...
  export default {
    ...
    methods: {
      ...
      handleDelDataCar() {
        svoPlanReceivecarApi({ids: this.checkedChange('3')}).then(res => {
          this.loadList();
          this.feedMessage('warning', res.repData)
        })
      },
      ...
      deltFactory() {
        if(this.checkedChange()){
          confirmPop('确认删除', () => {
            svoPlanDeleteApi({ids: this.checkedChange()}).then(res => {
              this.loadList()
              this.feedMessage('warning', res.repData);
            })
          })
      ...
      fromVal(num) {
        let self = this;
        console.log(self.dataFm.formData, 'form--')
        var p = {
