...
<script>
  ...
  import {ppoQueryAllPreApi, ppoCreateApi, ppoExportPreApi} from "@/common/config/api/PlanManage"
    ...
    methods: {
      ...
      svoVisibleConfirm(){
        ...
        if(this.isSaveflag) {
          ...
          console.log(this.checkedChange(), p)
          ppoCreateApi(p).then(res => {
      ...
      //列表
      svoLoadList(){
       ppoQueryAllPreApi(this.fromVal(1)).then(res => {
        if(res.repData) {
      ...
      exportBtn() {
        ppoExportPreApi(this.fromVal(2)).then(res => {
          if(res.repData){
