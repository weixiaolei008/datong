		...
		<el-header style="height: auto; padding: 0;">
			...
			<div class="search-group clearfix">
				...
				<el-collapse-transition>
					<div v-show="pop" class="search-pop clearfix">
						...
						<div class="input-box">
							...
							<el-select v-model="transportWay" placeholder="请选择运输方式">
								<el-option v-for="item in options.transport_way" :key="item.code" :label="item.name" :value="item.code">
								</el-option>
							</el-select>
						</div>
						<!-- 单选下拉框 -->
						...
						<!-- 时间 -->
						<div class="input-box">
							<label class="input-label">预计借车时间</label>
							<el-date-picker v-model="expectBorrow"  value-format="yyyy-MM-dd HH-mm-ss" type="datetimerange" range-separator="至" start-placeholder="开始日期"
							 end-placeholder="结束日期">
							</el-date-picker>
						</div>
						<!-- 时间 -->
						<div class="input-box">
							<label class="input-label">库改结束时间</label>
							<el-date-picker v-model="expectReturn"  value-format="yyyy-MM-dd HH-mm-ss" type="datetimerange" range-separator="至" start-placeholder="开始日期"
							 end-placeholder="结束日期">
							</el-date-picker>
						</div>
					</div>
				...
				<div class="input-box">
					...
					<el-date-picker v-model="commond" type="datetimerange" value-format="yyyy-MM-dd HH-mm-ss" range-separator="至"
			...
			<div class="btn-group clearfix">
				<div class="btn-add" @click="setup"><span class="img-add">执行计划</span></div>
	...
	export default {
		...
		methods: {
			...
			exportExcel() {
				...
				if (this.listIds.length) {
					...
					p = {
						...
						"listType": 2, // 列表类型 1 出库 2 预出库计划 3 入库,
			...
			search() {
				...
				if (this.expectBorrow == null) {
					this.expectBorrow = []
				}
				if (this.expectReturn == null) {
					this.expectReturn = []
				}
				this.loadData()
			},
			...
			setup() {
				...
				for (var i = 0; i < this.listIds.length; i++) {
					ids.push(this.listIds[i].ttPlanReceivecarId)
				}
				this.$confirm('请确认是否创建计划?', '提示', {
