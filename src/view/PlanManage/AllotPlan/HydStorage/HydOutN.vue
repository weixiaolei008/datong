<!--计划管理／库间调拨计划管理／氢能源车辆入库计划／氢能源车辆出库计划(未创建)-->
<template>
  <el-container style="width: 100%;height: 100%;">
    ...
    <el-dialog
      ...
      <div class="search-group clearfix">
        ...
        <div class="input-box">
          <label class="input-label">当前仓库</label>
          <el-input v-model="origWarehouse" :disabled="isDisabled" ></el-input>
        </div>
        <div class="input-box">
...
<script>
  ...
  import {hydQueryAllApi, hydInserttApi, hydExportApi} from "@/common/config/api/PlanManage"
    ...
    methods: {
      ...
      svoVisibleConfirm(){
        ...
        if(this.isSaveflag) {
          ...
          let p = {
            ids: this.checkedChange(),//当前仓库ID
            transportWay: this.transportWayId //运输方式
          }
          hydInserttApi(p).then(res => {
      ...
      addSvoConBtn() {
        if(this.checkedChange()){
          let isSame = true;
          let vins = [];
          let wareID = this.tableSelectId[0].currentWarehouseId;
          if(this.tableSelectId.length > 0){
            this.tableSelectId.map((item, index) => {
              if(wareID != item.currentWarehouseId) {
                isSame = false;
              }
              vins.push(item.vin);
            })
            ...
            this.origWarehouse =  this.tableSelectId[0].currentWarehouseName;
        ...
        if(this.tableSelectId.length > 0) {
          this.tableSelectId.map((item, index) => {
           if(item.status != '待执行'){//1:待执行2:已执行3:已执行
            ...
            ids.push(item.ttPlanHydrogenvehicleId);
          })
        }
      ...
      fromVal(num) {
        var p = {
          ...
          origWarehouseId: this.hydFrom.origWarehouseId,//始发仓库
          pointLocation: this.hydFrom.pointLocation,//点位信息
          salesArea: this.hydFrom.salesArea,//销售区域
          status: this.hydFrom.status,//状态 1:待执行2:已执行3:已执行
