    ...
    <el-dialog  title="创建计划" :visible.sync="popCreateVisible" width="380px" center>
      <div class="clearfix">
        <div class="input-box">
          ...
          <el-input v-model="popFormData.exportCountry" placeholder="请输入国家" clearable></el-input>
        </div>
        <div class="input-box">
          <label class="input-label">最迟出库时间</label>
          <el-date-picker
            v-model="popFormData.latestOutboundDate"
            placeholder="选择日期时间"
            type="datetime">
          </el-date-picker>
        </div>
        <div class="input-box">
          <label class="input-label">船期</label>
          <el-date-picker
            v-model="popFormData.shipDate"
            placeholder="选择日期时间"
            type="datetime">
          </el-date-picker>
        </div>
...
<script>
  import {loadCarOutStorageN, importCarOutStorage, exportOverseasCarN, creatOverseaPlan, holdOutStorage, holdOutStorageN, delCarOutStorage} from "@/common/config/api/PlanManage"
  ...
  export default {
    ...
    methods: {
      ...
      handleCreate(){
        ...
        this.popFormData.exportCountry = ''
        this.popFormData.latestOutboundDate = ''
        this.popFormData.shipDate = ''
      ...
      saveCreate() {
        ...
        if(self.isSaveFlag){
          ...
          let msg = {
            "exportCountry": '请选择国家',
            "latestOutboundDate": '请选择最迟出库时间',
            "shipDate": '请选择船期',
          }
          let formKeys = Object.keys(self.popFormData);
          for (let k of formKeys){
            if(!self.popFormData[k]){
              this.msgInfo(msg[k])
              return;
            }
          }
          ...
          let p = {
            "exportCountry": self.popFormData.exportCountry,
            "latestOutboundDate": self.popFormData.latestOutboundDate,
            "shipDate": self.popFormData.shipDate,
            "ids": ids
          }
          creatOverseaPlan(p).then(res => {
            res && res.success && self.msgInfo(res.repData);
            this.popCreateVisible = false;
            this.loadData();
          })
